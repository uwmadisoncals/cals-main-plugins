...
if ( ! class_exists( 'BF_Admin_Page_Class_Pro') ) :
 ...
 */
  ...
  class BF_Admin_Page_Class_Pro {
   ...
   // var_dump($this->google_fonts);
    ...
    //  $this->google_fonts = isset($args['google_fonts'])? true : false;
     ...
     add_action( 'wp_ajax_apc_migration_'.$this->option_group, array( $this, 'clt_migration_handler' ) );
    ...
    public function AddMenuTopPage() {
      ...
      $default = array(
        ...
        'icon_url'   => CTP_PLUGIN_URL.'/images/timeline-icon-222.png',
     ...
     // add_submenu_page('edit.php?post_type=cool_timeline',$this->args['page_title'],$this->args['page_title'], 'manage_options',$this->args['id'],  array($this, 'DisplayPage') );
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        echo '<img class="timeline_logo" src="'.CTP_PLUGIN_URL.'/images/cooltimeline-logo.jpg">'."\n".((isset($this->args['page_header_text']))? $this->args['page_header_text'] : '').'

        </div><div class="support_links" ><strong>V '.CTLPV.'</strong></div>'."\n".'
      ...
      $skip = array('title','paragraph','subtitle','TABS','CloseDiv','TABS_Listing','OpenTab','custom','import_export','content_migration');
  ...
  <h2 class="ctl_add_head">Please Give Us Your Valuable Feedback.</h2>

  <p>If you find our plugin and support helpful.<br>
  ...
  public function show_field_radio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      foreach ( $field['options'] as $key => $value ) {
        ...
        echo "<span class='radio-cont-{$field['id']}'><input type='radio' class='at-radio".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <span class='at-radio-label'>{$value}</span></span>"."\n";
...
//    $html .= $op. '</select>'."\n";
 ...
 $new_field = array('type' => 'content_migration','id'=> '','value' => '');
  ...
  {
    $ctl_can_migrate=get_option('ctl-can-migrate');
    $this->show_field_begin(array('name' => ''),null);
     $ret ='<div class="apc_ie_panel field">
    ...
    $ret .='<p>'.__('Migrate Stories From Free To Pro Version','cool-timeline').'</p><input class="button" type="button" value="'. __('Migrate content','cool-timeline').'" id="apc_migration_b" />';
     ...
     $ret .='<div class="migration_results alert" style="display: none;">
      ...
      <input type="hidden" id="option_group_name_2" value="'.$this->option_group.'" />
    <input type="hidden" id="ctl_can_migrate" value="'.$ctl_can_migrate.'" />
      <input type="hidden" id="apc_migration_nonce" name="apc_migration" value="'.wp_create_nonce("apc_migration").'" />';
    echo $ret;
    $this->show_field_end(array('name' => '','desc' => ''),null);  
  }
  ...
  public function import(){
    ...
    function clt_migration_handler(){
    
    $e=check_ajax_referer( 'apc_migration', 'seq' );
  
    $can_migrate= $_POST['can_migrate'];
    $group=$_POST['group'];
    $seq=$_POST['seq']; 
       ...
       foreach ( $posts as $post )
              {
               $published_date= get_the_date('m/d/Y H:i a', $post->ID );
                 $published_year= get_the_date('Y', $post->ID );
                  if($published_date){

...
bindOn('click','#apc_migration_b',function(){do_ajax_migration('migration');});
...
function do_ajax_migration(which){

   var group = jQuery("#option_group_name_2").val();
  var seq_selector = "#apc_migration_nonce";
  var action_selctor = "apc_migration_" + group;
 ...
 jQuery.post(ajaxurl,
    {
      ...
      action: action_selctor,
      seq: jQuery(seq_selector).val(),
      can_migrate: jQuery("#ctl_can_migrate").val(),
    },
    function(data) {
      if (data){
        if(data.err){
        jQuery(".migration_results").html(data.err).addClass('alert-warning').show('slow');
      }
        if (data.success){
          jQuery(".migration_results").html(data.success).addClass('alert-success').show('slow');
        }
      }else{

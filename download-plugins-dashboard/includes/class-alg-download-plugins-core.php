...
class Alg_Download_Plugins_Core {
	...
	function add_theme_download_links() {
		wp_enqueue_script(  'alg-theme-download-links', alg_download_plugins()->plugin_url() . '/includes/js/theme_download_link.js', array( 'jquery' ),
			alg_download_plugins()->version, true );
	...
	function download_plugin_all( $is_cron = false ) {
		if ( isset( $_GET['alg_download_plugin_all'] ) && is_user_logged_in() && current_user_can( 'activate_plugins' )  ) {
			...
			$zip_file_path        = sys_get_temp_dir() . '/' . $zip_file_name;
			$plugin_or_theme_path = WP_PLUGIN_DIR;
			$exclude_path         = $plugin_or_theme_path;
			$args                 = array( 'zip_file_path' => $zip_file_path, 'exclude_path' => $exclude_path );
			$files                = $this->get_files( $plugin_or_theme_path );
			if ( $this->create_zip( $args, $files ) ) {
	...
	function cron_schedule_themes_event() {
		if ( '' != ( $interval = apply_filters( 'alg_download_plugins', '', 'value_themes_bulk_period' ) ) && ! wp_next_scheduled( 'alg_download_themes_cron' ) ) {
	...
	function cron_schedule_plugins_event() {
		if ( '' != ( $interval = apply_filters( 'alg_download_plugins', '', 'value_plugins_bulk_period' ) ) && ! wp_next_scheduled( 'alg_download_plugins_cron' ) ) {

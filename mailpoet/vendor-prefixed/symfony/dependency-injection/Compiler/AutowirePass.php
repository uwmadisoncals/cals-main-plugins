...
use MailPoetVendor\Symfony\Component\DependencyInjection\Config\AutowireServiceResource;
use MailPoetVendor\Symfony\Component\DependencyInjection\ContainerBuilder;
use MailPoetVendor\Symfony\Component\DependencyInjection\Definition;
use MailPoetVendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException;
...
class AutowirePass extends \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public static function createResourceForClass(\ReflectionClass $reflectionClass)
    {
        ...
        return new \MailPoetVendor\Symfony\Component\DependencyInjection\Config\AutowireServiceResource($reflectionClass->name, $reflectionClass->getFileName(), $metadata);
    }
    /**
     * {@inheritdoc}
     */
    protected function processValue($value, $isRoot = \false)
    {
        ...
        } catch (\MailPoetVendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException $e) {
    ...
    private function doProcessValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Definition || !$value->isAutowired() || $value->isAbstract() || !$value->getClass()) {
            return $value;
        }
    ...
    private function autowireMethod(\ReflectionFunctionAbstract $reflectionMethod, array $arguments)
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            $type = \MailPoetVendor\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
            if (!$type) {
                ...
                if (!$parameter->isDefaultValueAvailable()) {
                    ...
                    $type = \MailPoetVendor\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \false);
                    $type = $type ? \sprintf('is type-hinted "%s"', $type) : 'has no type-hint';
                    throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, \sprintf('Cannot autowire service "%s": argument "$%s" of method "%s()" %s, you should configure its value explicitly.', $this->currentId, $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method, $type));
                }
                // specifically pass the default value
                $arguments[$index] = $parameter->getDefaultValue();
                continue;
            }
            if (!($value = $this->getAutowiredReference($ref = new \MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference($type, $type, !$parameter->isOptional() ? $class : ''), 'for ' . \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class . '::' . $method)))) {
                $failureMessage = $this->createTypeNotFoundMessage($ref, \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method));
                if ($parameter->isDefaultValueAvailable()) {
                    $value = $parameter->getDefaultValue();
                } elseif (!$parameter->allowsNull()) {
                    throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $failureMessage);
                }
     ...
     */
    private function getAutowiredReference(\MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference $reference, $deprecationMessage)
    {
        ...
        if (isset($this->definedTypes[$type])) {
            return new \MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference($this->types[$type], $type);
        }
        ...
        if (isset($this->autowired[$type])) {
            return $this->autowired[$type] ? new \MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference($this->autowired[$type], $type) : null;
        }
        if (!$this->strictMode) {
     ...
     */
    private function populateAvailableType($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition, $onlyAutowiringTypes)
    {
    ...
    private function createAutowiredDefinition($type)
    {
        ...
        $argumentDefinition = new \MailPoetVendor\Symfony\Component\DependencyInjection\Definition($type);
        ...
        return new \MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference($argumentId, $type);
    }
    ...
    private function createTypeAlternatives(\MailPoetVendor\Symfony\Component\DependencyInjection\TypedReference $reference)
    {

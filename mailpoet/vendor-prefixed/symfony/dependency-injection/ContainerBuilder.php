...
use MailPoetVendor\Symfony\Component\EventDispatcher\ContainerAwareEventDispatcher;
 ...
 */
class ContainerBuilder extends \MailPoetVendor\Symfony\Component\DependencyInjection\Container implements \MailPoetVendor\Symfony\Component\DependencyInjection\TaggedContainerInterface
{
    /**
    ...
    public function __construct(\MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\ParameterBagInterface $parameterBag = null)
    {
        ...
        $this->trackResources = \interface_exists('MailPoetVendor\\Symfony\\Component\\Config\\Resource\\ResourceInterface');
        $this->setDefinition('service_container', (new \MailPoetVendor\Symfony\Component\DependencyInjection\Definition(\MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::class))->setSynthetic(\true)->setPublic(\true));
        $this->setAlias(\MailPoetVendor\Psr\Container\ContainerInterface::class, new \MailPoetVendor\Symfony\Component\DependencyInjection\Alias('service_container', \false));
        $this->setAlias(\MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::class, new \MailPoetVendor\Symfony\Component\DependencyInjection\Alias('service_container', \false));
    }
    /**
     ...
     */
    public function setProxyInstantiator(\MailPoetVendor\Symfony\Component\DependencyInjection\LazyProxy\Instantiator\InstantiatorInterface $proxyInstantiator)
    {
    ...
    public function getExtension($name)
    {
        ...
        throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\LogicException(\sprintf('Container extension "%s" is not registered', $name));
    }
    /**
     ...
     * @return $this
     */
    public function addResource(\MailPoetVendor\Symfony\Component\Config\Resource\ResourceInterface $resource)
    {
        if (!$this->trackResources) {
            return $this;
        }
        if ($resource instanceof \MailPoetVendor\Symfony\Component\Config\Resource\GlobResource && $this->inVendors($resource->getPrefix())) {
            return $this;
        }
    ...
    public function getReflectionClass($class, $throw = \true)
    {
        ...
        if ($this->trackResources) {
            if (!$classReflector) {
                $this->addResource($resource ?: new \MailPoetVendor\Symfony\Component\Config\Resource\ClassExistenceResource($class, \false));
            } elseif (!$classReflector->isInternal()) {
                $path = $classReflector->getFileName();
                if (!$this->inVendors($path)) {
                    $this->addResource(new \MailPoetVendor\Symfony\Component\Config\Resource\ReflectionClassResource($classReflector, $this->vendors));
                }
            }
    ...
    public function fileExists($path, $trackContents = \true)
    {
        ...
        if (!$exists) {
            $this->addResource(new \MailPoetVendor\Symfony\Component\Config\Resource\FileExistenceResource($path));
            return $exists;
        }
        if (\is_dir($path)) {
            if ($trackContents) {
                $this->addResource(new \MailPoetVendor\Symfony\Component\Config\Resource\DirectoryResource($path, \is_string($trackContents) ? $trackContents : null));
            } else {
                $this->addResource(new \MailPoetVendor\Symfony\Component\Config\Resource\GlobResource($path, '/*', \false));
            }
        } elseif ($trackContents) {
            $this->addResource(new \MailPoetVendor\Symfony\Component\Config\Resource\FileResource($path));
        }
        return $exists;
    }
    /**
    ...
    public function getCompiler()
    {
        if (null === $this->compiler) {
            $this->compiler = new \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\Compiler();
        }
    ...
    public function set($id, $service)
    {
        ...
        if ($this->isCompiled() && (isset($this->definitions[$id]) && !$this->definitions[$id]->isSynthetic())) {
            ...
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\BadMethodCallException(\sprintf('Setting service "%s" for an unknown or non-synthetic service definition on a compiled container is not allowed.', $id));
        }
     ...
     * @see Reference
     */
    public function get($id, $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE)
    {
    ...
    private function doGet($id, $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE, array &$inlineServices = null, $isConstructorArgument = \false)
    {
        $id = $this->normalizeId($id);
        ...
        try {
            ...
            if ($service = parent::get($id, \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE)) {
                return $service;
            }
        ...
        } catch (\MailPoetVendor\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException $e) {
            if (\MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $invalidBehavior) {
                return;
            }
            throw $e;
        }
    ...
    {
        if ($this->isCompiled()) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\BadMethodCallException('Cannot merge on a compiled container.');
        }
        ...
        if ($this->getParameterBag() instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag && $container->getParameterBag() instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag) {
        ...
        foreach ($container->getAutoconfiguredInstanceof() as $interface => $childDefinition) {
            if (isset($this->autoconfiguredInstanceof[$interface])) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" has already been autoconfigured and merge() does not support merging autoconfiguration for the same class/interface.', $interface));
            }
    ...
    public function compile()
    {
        ...
        if ($resolveEnvPlaceholders && $bag instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag) {
            $compiler->addPass(new \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\ResolveEnvPlaceholdersPass(), \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\PassConfig::TYPE_AFTER_REMOVING, -1000);
        }
    ...
    public function setAlias($alias, $id)
    {
        ...
        if (\is_string($id)) {
            ...
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('$id must be a string, or an Alias object.');
        }
        if ($alias === (string) $id) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('An alias can not reference itself, got a circular reference on "%s".', $alias));
        }
    ...
    public function getAlias($id)
    {
        ...
        if (!isset($this->aliasDefinitions[$id])) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The service alias "%s" does not exist.', $id));
        }
    ...
    {
        return $this->setDefinition($id, new \MailPoetVendor\Symfony\Component\DependencyInjection\Definition($class));
    }
    /**
    ...
    {
        return $this->setDefinition($id, (new \MailPoetVendor\Symfony\Component\DependencyInjection\Definition($class))->setAutowired(\true));
    }
    /**
     ...
     * @throws BadMethodCallException When this ContainerBuilder is compiled
     */
    public function setDefinition($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition)
    {
        if ($this->isCompiled()) {
    ...
    public function findDefinition($id)
    {
        ...
        while (isset($this->aliasDefinitions[$id])) {
            ...
            if (isset($seen[$id])) {
                ...
                $seen[] = $id;
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($id, $seen);
            }
            $seen[$id] = $id;
        }
     ...
     */
    private function createService(\MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition, array &$inlineServices, $isConstructorArgument = \false, $id = null, $tryProxy = \true)
    {
        if (null === $id && isset($inlineServices[$h = \spl_object_hash($definition)])) {
            return $inlineServices[$h];
        }
        if ($definition instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Constructing service "%s" from a parent definition is not supported at build time.', $id));
        }
        if ($definition->isSynthetic()) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('You have requested a synthetic service ("%s"). The DIC does not know how to construct this service.', $id));
        }
        if ($definition->isDeprecated()) {
            @\trigger_error($definition->getDeprecationMessage($id), \E_USER_DEPRECATED);
        }
        if ($tryProxy && $definition->isLazy() && !($tryProxy = !($proxy = $this->proxyInstantiator) || $proxy instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\LazyProxy\Instantiator\RealServiceInstantiator)) {
        ...
        if ($callable = $definition->getConfigurator()) {
            if (\is_array($callable)) {
                ...
                } elseif ($callable[0] instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Definition) {
                    $callable[0] = $this->createService($callable[0], $inlineServices);
                }
            }
            if (!\is_callable($callable)) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The configure callable for class "%s" is not a callable.', \get_class($service)));
            }
    ...
    private function doResolveServices($value, array &$inlineServices = [], $isConstructorArgument = \false)
    {
        ...
        } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
            $reference = $value->getValues()[0];
            $value = function () use($reference) {
                return $this->resolveServices($reference);
            };
        } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
            $value = new \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\RewindableGenerator(function () use($value) {
                foreach ($value->getValues() as $k => $v) {
                    ...
                    foreach (self::getInitializedConditionals($v) as $s) {
                        if (!$this->doGet($s, \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE)) {
                            continue 2;
                        }
                    }
        ...
        foreach ($this->getDefinitions() as $id => $definition) {
            if ($definition->hasTag($name)) {
                if ($throwOnAbstract && $definition->isAbstract()) {
                    throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The service "%s" tagged "%s" must not be abstract.', $id, $name));
                }
    ...
    public function addExpressionLanguageProvider(\MailPoetVendor\Symfony\Component\ExpressionLanguage\ExpressionFunctionProviderInterface $provider)
    {
    ...
    {
        if (!isset($this->autoconfiguredInstanceof[$interface])) {
            $this->autoconfiguredInstanceof[$interface] = new \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition('');
        }
    ...
    public function resolveEnvPlaceholders($value, $format = null, array &$usedEnvs = null)
    {
        ...
        $envPlaceholders = $bag instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $bag->getEnvPlaceholders() : $this->envPlaceholders;
        $completed = \false;
                ...
                if (\false !== \stripos($value, $placeholder)) {
                    ...
                    if ($placeholder === $value) {
                        ...
                        if (!\is_string($resolved) && !\is_numeric($resolved)) {
                            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('A string value must be composed of strings and/or numbers, but found parameter "env(%s)" of type %s inside string value "%s".', $env, \gettype($resolved), $this->resolveEnvPlaceholders($value)));
                        }
     ...
     * @final
     */
    public function log(\MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface $pass, $message)
    {
    ...
    public static function getServiceConditionals($value)
    {
        ...
        } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference && \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE === $value->getInvalidBehavior()) {
    ...
    public static function getInitializedConditionals($value)
    {
        ...
        } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference && \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $value->getInvalidBehavior()) {
            $services[] = (string) $value;
        }
        return $services;
    }
    ...
    protected function getEnv($name)
    {
        ...
        $bag = $this->getParameterBag();
        if (!\is_string($value) || !$bag instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag) {
            return $value;
        }
        foreach ($bag->getEnvPlaceholders() as $env => $placeholders) {
            if (isset($placeholders[$value])) {
                $bag = new \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\ParameterBag($bag->all());
    ...
    private function callMethod($service, $call, array &$inlineServices)
    {
        ...
        foreach (self::getInitializedConditionals($call[1]) as $s) {
            if (!$this->doGet($s, \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE, $inlineServices)) {
                return;
            }
        }
     ...
     */
    private function shareService(\MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition, $service, $id, array &$inlineServices)
    {
    ...
    private function inVendors($path)
    {
        if (null === $this->vendors) {
            $resource = new \MailPoetVendor\Symfony\Component\Config\Resource\ComposerResource();

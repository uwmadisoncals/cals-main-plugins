 ...
 */
class XmlFileLoader extends \MailPoetVendor\Symfony\Component\DependencyInjection\Loader\FileLoader
{
    ...
    private function parseImports(\DOMDocument $xml, $file)
    {
        ...
        foreach ($imports as $import) {
            ...
            $this->import($import->getAttribute('resource'), \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('type')) ?: null, (bool) \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('ignore-errors')), $file);
        }
    }
    /**
    ...
    private function getServiceDefaults(\DOMDocument $xml, $file)
    {
        ...
        if ($defaultsNode->hasAttribute('autowire')) {
            $defaults['autowire'] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($defaultsNode->getAttribute('autowire'));
        }
        if ($defaultsNode->hasAttribute('public')) {
            $defaults['public'] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($defaultsNode->getAttribute('public'));
        }
        if ($defaultsNode->hasAttribute('autoconfigure')) {
            $defaults['autoconfigure'] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($defaultsNode->getAttribute('autoconfigure'));
        }
        return $defaults;
    }
    /**
    ...
    private function parseDefinition(\DOMElement $service, $file, array $defaults)
    {
        if ($alias = $service->getAttribute('alias')) {
            ...
            $this->container->setAlias((string) $service->getAttribute('id'), $alias = new \MailPoetVendor\Symfony\Component\DependencyInjection\Alias($alias));
            if ($publicAttr = $service->getAttribute('public')) {
                $alias->setPublic(\MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($publicAttr));
        ...
        if ($this->isLoadingInstanceof) {
            $definition = new \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition('');
        } elseif ($parent = $service->getAttribute('parent')) {
            if (!empty($this->instanceof)) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The service "%s" cannot use the "parent" option in the same file where "instanceof" configuration is defined as using both is not supported. Move your child definitions to a separate file.', $service->getAttribute('id')));
            }
            foreach ($defaults as $k => $v) {
                ...
                if ('bind' === $k) {
                    if ($defaults['bind']) {
                        throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Bound values on service "%s" cannot be inherited from "defaults" when a "parent" is set. Move your child definitions to a separate file.', $service->getAttribute('id')));
                    }
                    continue;
                }
                if (!$service->hasAttribute($k)) {
                    throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Attribute "%s" on service "%s" cannot be inherited from "defaults" when a "parent" is set. Move your child definitions to a separate file or define this attribute explicitly.', $k, $service->getAttribute('id')));
                }
            }
            $definition = new \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition($parent);
        } else {
        ...
        if ($value = $service->getAttribute('autowire')) {
            $definition->setAutowired(\MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($value));
        }
        if ($value = $service->getAttribute('autoconfigure')) {
            if (!$definition instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition) {
                $definition->setAutoconfigured(\MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($value));
            } elseif ($value = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($value)) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The service "%s" cannot have a "parent" and also have "autoconfigure". Try setting autoconfigure="false" for the service.', $service->getAttribute('id')));
            }
        }
        ...
        $definition->setArguments($this->getArgumentsAsPhp($service, 'argument', $file, \false, $definition instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition));
        ...
        if ($factories = $this->getChildren($service, 'factory')) {
            ...
            if ($function = $factory->getAttribute('function')) {
                ...
                if ($childService = $factory->getAttribute('service')) {
                    $class = new \MailPoetVendor\Symfony\Component\DependencyInjection\Reference($childService, \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE);
                } else {
        ...
        foreach ($tags as $tag) {
            ...
            foreach ($tag->attributes as $name => $node) {
                ...
                if (\false !== \strpos($name, '-') && \false === \strpos($name, '_') && !\array_key_exists($normalizedName = \str_replace('-', '_', $name), $parameters)) {
                    $parameters[$normalizedName] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
                }
                // keep not normalized key
                $parameters[$name] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
            }
            if ('' === $tag->getAttribute('name')) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The tag name for service "%s" in %s must be a non-empty string.', (string) $service->getAttribute('id'), $file));
            }
    ...
    private function getArgumentsAsPhp(\DOMElement $node, $name, $file, $lowercase = \true, $isChildDefinition = \false)
    {
        ...
        foreach ($this->getChildren($node, $name) as $arg) {
            ...
            $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE;
            if ('ignore' == $onInvalid) {
                $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE;
            } elseif ('ignore_uninitialized' == $onInvalid) {
                $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE;
            } elseif ('null' == $onInvalid) {
                $invalidBehavior = \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE;
            }
            switch ($arg->getAttribute('type')) {
                case 'service':
                    ...
                    $arguments[$key] = new \MailPoetVendor\Symfony\Component\DependencyInjection\Reference($arg->getAttribute('id'), $invalidBehavior);
                    break;
                case 'expression':
                    if (!\class_exists(\MailPoetVendor\Symfony\Component\ExpressionLanguage\Expression::class)) {
                    ...
                    $arguments[$key] = new \MailPoetVendor\Symfony\Component\ExpressionLanguage\Expression($arg->nodeValue);
                    break;
                case 'collection':
                ...
                case 'iterator':
                    ...
                    try {
                        $arguments[$key] = new \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\IteratorArgument($arg);
                    } catch (\MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException $e) {
                        throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="iterator" only accepts collections of type="service" references in "%s".', $name, $file));
                    }
                    break;
                case 'tagged':
                    if (!$arg->getAttribute('tag')) {
                        throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="tagged" has no or empty "tag" attribute in "%s".', $name, $file));
                    }
                    $arguments[$key] = new \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument($arg->getAttribute('tag'));
                    break;
                case 'string':
                ...
                case 'constant':
                    ...
                    break;
                default:
                    $arguments[$key] = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($arg->nodeValue);
            }
        }
        return $arguments;
    }
...
EOF;
    ...
    private function validateExtensions(\DOMDocument $dom, $file)
    {
        foreach ($dom->documentElement->childNodes as $node) {
            ...
            if (!$this->container->hasExtension($node->namespaceURI)) {
                ...
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('There is no extension able to load the configuration for "%s" (in %s). Looked for namespace "%s", found %s', $node->tagName, $file, $node->namespaceURI, $extensionNamespaces ? \sprintf('"%s"', \implode('", "', $extensionNamespaces)) : 'none'));
            }
        }
    }
    /**
    ...
    {
        return \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::convertDomElementToArray($element);
    }
}

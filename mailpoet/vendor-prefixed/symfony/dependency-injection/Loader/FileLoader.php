...
use MailPoetVendor\Symfony\Component\Config\FileLocatorInterface;
use MailPoetVendor\Symfony\Component\Config\Loader\FileLoader as BaseFileLoader;
 ...
 */
abstract class FileLoader extends \MailPoetVendor\Symfony\Component\Config\Loader\FileLoader
{
    protected $container;
     ...
     */
    public function registerClasses(\MailPoetVendor\Symfony\Component\DependencyInjection\Definition $prototype, $namespace, $resource, $exclude = null)
    {
        if ('\\' !== \substr($namespace, -1)) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Namespace prefix must end with a "\\": %s.', $namespace));
        }
        if (!\preg_match('/^(?:[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*+\\\\)++$/', $namespace)) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Namespace is not a valid PSR-4 prefix: %s.', $namespace));
        }
    ...
    protected function setDefinition($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition)
    {
        if ($this->isLoadingInstanceof) {
            ...
            $this->container->setDefinition($id, $definition instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ChildDefinition ? $definition : $definition->setInstanceofConditionals($this->instanceof));
        }
    }
    private function findClasses($namespace, $pattern, $excludePattern)
    {
        ...
        if ($excludePattern) {
            ...
            foreach ($this->glob($excludePattern, \true, $resource) as $path => $info) {
        ...
        foreach ($this->glob($pattern, \true, $resource) as $path => $info) {
            if (null === $prefixLen) {
                ...
                if ($excludePrefix && 0 !== \strpos($excludePrefix, $resource->getPrefix())) {
                    throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid "exclude" pattern when importing classes for "%s": make sure your "exclude" pattern (%s) is a subset of the "resource" pattern (%s)', $namespace, $excludePattern, $pattern));
                }
            }
            ...
            if (!$r) {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Expected to find class "%s" in file "%s" while importing services from resource "%s", but it was not found! Check the namespace prefix used with the resource.', $class, $path, $pattern));
            }
        ...
        if ($resource instanceof \MailPoetVendor\Symfony\Component\Config\Resource\GlobResource) {

 ...
 */
class XmlDumper extends \MailPoetVendor\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
     * @var \DOMDocument
     */
    ...
    private function addService($definition, $id, \DOMElement $parent)
    {
        ...
        if ($callable = $definition->getFactory()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Definition) {
                $this->addService($callable[0], null, $factory);
                $factory->setAttribute('method', $callable[1]);
            } elseif (\is_array($callable)) {
                if (null !== $callable[0]) {
                    $factory->setAttribute($callable[0] instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                }
                $factory->setAttribute('method', $callable[1]);
            } else {
        ...
        if ($callable = $definition->getConfigurator()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Definition) {
                ...
                $configurator->setAttribute($callable[0] instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                $configurator->setAttribute('method', $callable[1]);
            } else {
     ...
     * @param \DOMElement $parent
     */
    private function addServiceAlias($alias, \MailPoetVendor\Symfony\Component\DependencyInjection\Alias $id, \DOMElement $parent)
    {
        $service = $this->document->createElement('service');
    ...
    private function convertParameters(array $parameters, $type, \DOMElement $parent, $keyAttribute = 'key')
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {

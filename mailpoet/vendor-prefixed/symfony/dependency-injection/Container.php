...
class Container implements \MailPoetVendor\Symfony\Component\DependencyInjection\ResettableContainerInterface
{
    ...
    {
        $this->parameterBag = $parameterBag ?: new \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag();
    }
    /**
    ...
    public function compile()
    {
        ...
        $this->parameterBag = new \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\FrozenParameterBag($this->parameterBag->all());
        $this->compiled = \true;
    }
    /**
    ...
    public function isFrozen()
    {
        ...
        return $this->parameterBag instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ParameterBag\FrozenParameterBag;
    }
    /**
    ...
    public function set($id, $service)
    {
        ...
        if ('service_container' === $id) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('You cannot set service "service_container".');
        }
    ...
    public function has($id)
    {
        for ($i = 2;;) {
            ...
            if (!$this->methodMap && !$this instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerBuilder && __CLASS__ !== static::class && \method_exists($this, 'get' . \strtr($id, $this->underscoreMap) . 'Service')) {
    ...
    public function get($id, $invalidBehavior = 1)
    {
        ...
        for ($i = 2;;) {
            ...
            try {
                if (isset($this->fileMap[$id])) {
                    ...
                    $id = $normalizedId;
                    continue;
                } elseif (!$this->methodMap && !$this instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerBuilder && __CLASS__ !== static::class && \method_exists($this, $method = 'get' . \strtr($id, $this->underscoreMap) . 'Service')) {
                    // We only check the convention-based factory in a compiled container (i.e. a child class other than a ContainerBuilder,
                    // and only when the dumper has not generated the method map (otherwise the method map is considered to be fully populated by the dumper)
                    @\trigger_error('Generating a dumped container without populating the method map is deprecated since Symfony 3.2 and will be unsupported in 4.0. Update your dumper to generate the method map.', \E_USER_DEPRECATED);
        ...
        if (1 === $invalidBehavior) {
            ...
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($id, null, null, $alternatives);
        }
    }
    /**
    ...
    protected function getEnv($name)
    {
        if (isset($this->resolving[$envName = "env({$name})"])) {
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\ParameterCircularReferenceException(\array_keys($this->resolving));
        }
        ...
        $processor = $processors->has($prefix) ? $processors->get($prefix) : new \MailPoetVendor\Symfony\Component\DependencyInjection\EnvVarProcessor($this);

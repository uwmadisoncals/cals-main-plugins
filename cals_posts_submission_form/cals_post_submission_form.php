<?php
/*
Plugin Name: CALS Posts Submissions Form
...
6.- Add [cals_posts_submissions_form] shortcode to body of page in which you want to display form 
	...
	class CALSPostsSubmissionsForm{
		...
		function CALSPostsSubmissionsForm(){		
			...
			$this->validation_rules = array('post_title' => 'required',
											...
											'submitter_email' => array('required' => true,
																	   'email' => true),
											'submitter_dept' => 'required',
											'submitter_dept_other' => 'required'
										   );
			
			//define categories to include or exclude
			$this->include_categories = array('agriculture',
							'food',
							'health',
							'environment',
							'energy',
							'communities'); //this could be accepted as a parameter
		...
		*/
		function create_form(){
			...
			<div id="cals_posts_submission_form" name="cals_posts_submission_form">
				...
				<p>(<span class="cals_psf_required_field"> * </span> = required field)</p>
				<form id="cals_psf" name="cals_psf" method = "post" action="'.get_bloginfo('url').'/'.$post->post_name.'/">
					<fieldset>
						...
						<ul>';
							...
							$form_html.= '<li>'.$this->create_form_field($params).'</li>';						$form_html.='
						<li><br/>
								<label for="post_cat_list" class="cals_psf_field_label">Categories: </label><br />';
		...
		function define_messages(){
			...
			$this->messages = array('user_already_logged_in' => '<strong>Hey there!</strong><br/><br/>
									...
									'success_save_data' => 'Thank you! Your post has been submitted successfully. <br/><br/>
									<a href="'.get_bloginfo('url').'/'.$post->post_title.'"><strong>Click here to submit a new post &raquo;</strong></a>',
		...
		*/
		...
		function get_form_categories(){
			...
			$categories = get_categories('get=all&hierarchical=1&include='.$include);
			
			$cat_content =  '<ul id="post_cat_list" name="post_cat_list" class="cals_psf_cat_list">';
			foreach ($categories as $cat){
				...
				$cat_content.= '<li id="'.$cat->cat_ID.'">
					<label class="selectit">
						<input value="'.$cat->cat_ID.'" name="post_category[]" id="in-category-'.$cat->cat_ID.'" type="checkbox" '.$checked.'>  '.ucwords($cat->category_nicename).'
					</label>
				</li>';
			}
			$cat_content.= '</ul>';
...
if (isset($cals_psf)) {
	...
	if(is_admin()){ //what the cals_psf plugin should run only in admin section
		...
		//Shortcode
		add_shortcode('cals_posts_submissions_form', array(&$cals_psf, 'get_form'));
	}
}
?>
...
class Cornerstone_Element_Orchestrator extends Cornerstone_Plugin_Component {
    ...
    $elements = CS()->get_registry('classic-elements', 'mk2' );
		...
		foreach ( $elements as $name ) {
			...
			$this->add( $class_name, $name, $this->path( "includes/elements/classic/$name") );
    ...
    $elements = CS()->get_registry('classic-elements', 'mk1' );

		foreach ( $elements as $name ) {

      $filename = $this->path( "includes/elements/classic/_alternate/$name.php" );

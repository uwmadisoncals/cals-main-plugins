...
class Ai1wmme_Import_Database {
	...
	public static function execute( $params ) {
		...
		foreach ( $blogs as $blog ) {
			...
			foreach ( $home_urls as $home_url ) {
				...
				if ( ! in_array( sprintf( "'%s'", trim( ai1wm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) ), $old_raw_values ) ) {
					...
					$new_raw_values[] = sprintf( "'%s'", get_blog_option( $blog['New']['BlogID'], 'upload_path' ) );
				}
		...
		if ( empty( $multisite['Network'] ) ) {
			...
			$new_prefixes[] = ai1wm_table_prefix( 'mainsite' );
			...
			foreach ( $blogs as $blog ) {
				if ( ai1wm_main_site( $blog['Old']['BlogID'] ) === true ) {
					foreach ( $wpdb->global_tables as $table_name ) {
						$old_prefixes[] = ai1wm_servmask_prefix() . $table_name;
						$new_prefixes[] = ai1wm_table_prefix( 'mainsite' ) . $table_name;
					}
				}
			}
		...
		if ( isset( $config['Plugin']['Version'] ) && ( $version = $config['Plugin']['Version'] ) ) {
			if ( $version !== 'develop' && version_compare( $version, '4.10', '<' ) ) {
				...
				foreach ( $blogs as $blog ) {
					$include_prefixes[] = ai1wm_table_prefix( $blog['New']['BlogID'] );
				}

				// Set include table prefixes
				if ( $include_prefixes ) {
					$mysql->set_include_table_prefixes( $include_prefixes );
					$mysql->flush();
				}
			}
		}

...
{
    static function is_super( $user, $super_roles, $super_caps )
    {
        $is_super = false;
    ...
    static function preserve_code( $text )
    {
        $ppn_allow_html = ppnFunctionCollection::get_settings( 'allow_html' );
    ...
    static function kses_data( $text )
    {
    ...
    static function prepare_js_from_kses( $text )
    {
    ...
    static function notes_column_content( $name )
    {
        ...
        switch( $name )
        {
            case 'ppn_notes':
                ...
                if( $latest_note )
                {
                    ...
                    print '[<em>' . mysql2date( __( 'M j, Y \a\t G:i', 'peters_post_notes' ), $latest_note->notetime ) . ', ' . $author_name . '</em>]<br />' . stripslashes( $latest_note->notecontent );
                }
                else
                {
                    print '-';
    ...
    */
    ...
    static function get_level_names()
    {
        global $wp_roles;
    ...
    static function get_role_names()
    {
        global $wp_roles;
        $role_names = array();

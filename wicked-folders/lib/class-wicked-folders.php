    ...
    public static function get_folders( $post_type, $taxonomy = false ) {
		...
		if ( ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				$cache_key[] = $term->term_id . '-' . $term->parent . '-' . $term->name;
			}
		...
		if ( self::dynamic_folders_enabled_for( $post_type ) ) {
			...
			$dynamic_folders = array_merge( $dynamic_folders, $unassigned, $author_folders, $date_folders, $term_folders, $hierarchy_folders );

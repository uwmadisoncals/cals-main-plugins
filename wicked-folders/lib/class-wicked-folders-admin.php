...
final class Wicked_Folders_Admin {
	...
	public function admin_notices() {
		...
		if ( $this->is_folder_pane_enabled_page() && ! in_array( 'toggle_folder_pane_hint', $dismissed_messages ) ) {
			$this->add_admin_notice( __( "Hint: The folders pane to the left can be toggled on or off.  To hide the folder pane, click 'Hide Folder Pane' under the 'Folders' menu item in the admin bar at the top of the screen.", 'wicked-folders' ) . '<a class="wicked-dismiss" href="#" data-key="toggle_folder_pane_hint">' . __( 'Dismiss', 'wicked-folders' ) . '</a>', 'notice notice-success wicked-dismissable' );
		}
	...
	public function get_screen_state( $screen_id = false, $user_id = false ) {
	...
	public function is_folder_pane_enabled_page() {
		...
		if ( Wicked_Folders::enabled_for( $typenow ) && ( 'edit' == $screen->base || 'upload' == $screen->base ) ) {
			if ( 'upload' == $screen->base && 'grid' == $mode ) {
				return false;
			}
			return true;
		}
	...
	public function save_settings() {
		...
		if ( 'wicked_folders_save_settings' == $action && wp_verify_nonce( $_REQUEST['nonce'], 'wicked_folders_save_settings' ) ) {
			...
			if ( 'general' == $tab ) {
				...
				update_option( 'wicked_folders_sync_upload_folder_dropdown', $sync_upload_folder_dropdown );
				update_option( 'wicked_folders_enable_folder_pages', $enable_folder_pages );
				update_option( 'wicked_folders_include_children', $include_children );
				update_option( 'wicked_folders_include_attachment_children', $include_attachment_children );
	...
	public function wp_before_admin_bar_render() {
		...
		if ( $this->is_folder_pane_enabled_page() ) {
			...
			$wp_admin_bar->add_node( array(
				...
				'id' 		=> 'wicked-folders-toggle-folder-pane',
				'title' 	=> __( ( $state->is_folder_pane_visible ? 'Hide Folder Pane' : 'Show Folder Pane' ), 'wicked-folders' ),
				'href' 		=> '#',
			) );

		}

		// Items for folders page
		if ( $this->is_folders_page() ) {
			...
			$wp_admin_bar->add_node( array(
				'parent' 	=> 'wicked-folders',
				'id' 		=> 'wicked-folders-clone-folder',
				'title' 	=> __( 'Clone Folder', 'wicked-folders' ),
				'href' 		=> $url,
			) );

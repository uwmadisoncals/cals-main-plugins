...
class VarnishMultiStatus {
	...
	function varnish_ip_callback() {
		...
		?>
		<input type="text" id="vhp_varnish_ip" name="vhp_varnish_ip" value="<?php echo $varnishArray; ?>" size="25" <?php if ( $disabled == true ) { echo 'disabled'; } ?>/>
		<label for="vhp_varnish_ip">
			<?php
			if ( $disabled == true ) { 
				_e('The Varnish IPs have been defined in your wp-config, so they are not editable here.', 'varnish-http-purge');
			} else {
				_e('Example:', 'varnish-http-purge'); ?> <code>123.45.67.89,87.65.43.21</code><?php
			}
			?>
		</label>
	<?php
	...
	function options_callback_url() {
		...
		$url = esc_url( VarnishMultiPurger::the_home_url() );
		...
		<?php
			...
			if ( $preflight == false ) {
				...
				?><tr>
					...
					<td><?php _e( 'Varnish 4 is running properly and caching is happening.', 'varnish-http-purge' ); ?></td>
				</tr><?php
				} elseif (isset( $headers['X-Cache'] ) && strpos( $headers['X-Cache'] ,'MISS') !== false ) {
				?><tr>
					<td width="40px"><?php echo $icon_bad; ?></td>
					<td><?php _e( 'Varnish 4 is running but cannot cache.', 'varnish-http-purge' ); ?></td>
				</tr><?php
				} else {
				?><tr>
					<td width="40px"><?php echo $icon_warning; ?></td>
					<td><?php _e( 'We did not find Varnish 4 active for this domain.', 'varnish-http-purge' ); ?></td>
				</tr><?php
				}
	...
	function varnish_url_callback() {
		...
		<label for="vhp_varnish_url">
			<?php printf( __( 'Example: <code>%s</code>', 'varnish-http-purge' ), esc_url( VarnishMultiPurger::the_home_url() ) ); ?>
		</label>
	<?php
	}
	...
	function varnish_url_sanitize( $input ) {
		...
		$baseurl_host = parse_url( esc_url( VarnishMultiPurger::the_home_url() ), PHP_URL_HOST );
		...
		if ( $baseurl_host == parse_url( $thisurl, PHP_URL_HOST ) ) {
			...
			$output = esc_url( VarnishMultiPurger::the_home_url() );

...
if( isset( $array_regions ) && is_array( $array_regions ) && !empty( $array_regions ) ) {
	...
	$output .= '<div class="rvm_regions_input rvm_regions_wrapper_hover_color"><label for="rvm_region_activate_on_mouse_over" ' . RVM_LABEL_CLASS . ' >' . __( 'Activate Mouse Over Background <br> <span class="rvm_small_text">hold  [SHIFT] key for multiple select</span>', RVM_TEXT_DOMAIN ) .  '</label><input  type="checkbox" class="rvm_region_checkbox rvm_region_checkbox_bg" name="' . strval( $region[ 1 ] ) . '[]"    ' . checked( 'checked', $regionsparams_array[ 'field_region_mouse_hover_over_colour' ] , false ) . ' ></div>';
	 ...
	 $output .= '<div class="rvm_regions_input rvm_regions_onclick_action"><label for="rvm_region_onclick_action" ' . RVM_LABEL_CLASS . ' >' . __( 'When click onto this subdivision: ', RVM_TEXT_DOMAIN ) .  '</label><select class="rvm_region_label_action" name="' . strval( $region[ 1 ] ) . '[]"><option ' . selected( 'open_link', $regionsparams_array[ 'field_region_onclick_action' ] , false ) . ' value="open_link">Open link</option><option ' . selected( 'open_label_onto_default_card', $regionsparams_array[ 'field_region_onclick_action' ] , false ) . ' value="open_label_onto_default_card" >Open label content onto default card</option><option ' . selected( 'show_custom_selector', $regionsparams_array[ 'field_region_onclick_action' ] , false ) . ' value="show_custom_selector">Show custom selector</option></select>';

...
/* Add Plugin Settings to the Dashboard */
...
function pexels_fsp_action_links( $links ) {
   $links[] = '<a href="'. esc_url( get_admin_url(null, 'options-general.php?page=pexels_fsp_images_settings') ) .'">Settings</a>';
   return $links;
}

/* Add the "Pexels Images" tab to the WP Media Uploader */
function pexels_fsp_media_upload_tabs_handler($tabs) {
	$tabs['pexels_fsp_tab'] = __('Pexels Images', 'pexels_fsp_images');
	return $tabs;
}
add_filter('media_upload_tabs', 'pexels_fsp_media_upload_tabs_handler');
...
function media_pexels_fsp_images_tab() {
	...
	<div style="padding:10px 15px 25px">
	   <form id="pexels_fsp_images_form" style="margin:0">
		 ...
		 <div style="clear:both;padding:12px 0 0;text-align:center">Clicking an image will download and add the image to your Media Library. Do not click an image just to "preview" it.</div>
	   <div id="pexels_fsp_results" class="flex-images" style="margin-top:20px;padding-top:25px;border-top:1px solid #ddd"></div>
		 <p style="text-align:center;">Photos provided by <a href="https://pexels.com/?utm_source=wordpress-plugin&utm_medium=post-editor" target="_blank" rel="noopener"><img src="<?= plugin_dir_url(__FILE__).'img/pexels-logo.png' ?>" style="margin:0 3px;position:relative;top:4px" width="80"></a></p>
	</div>
	<script>
	   /*
...
function media_upload_pexels_fsp_tab_handler() {
	wp_iframe('media_pexels_fsp_images_tab');
}
add_action('media_upload_pexels_fsp_tab', 'media_upload_pexels_fsp_tab_handler');

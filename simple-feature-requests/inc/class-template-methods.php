...
class JCK_SFR_Template_Methods
{
    ...
    public static function loop_item_author( $feature_request )
    {
        ...
        ?>
			<?php 
        printf( '%s %s', get_the_author_meta( 'user_nicename', $feature_request->post->post_author ), __( 'shared this idea', 'simple-feature-requests' ) );
        ?>
		</span>
		<?php 
    }
    ...
    public static function loop_item_comment_count( $feature_request )
    {
		...
		<span class="jck-sfr-comment-count jck-sfr-u-nowrap">
			<?php 
        printf( _n( '%d comment', '%d comments', $comment_count ), $comment_count );
        ?>
		</span>
		<?php 
    }
    ...
    {
        $href = apply_filters( 'jck_sfr_archive_link', get_post_type_archive_link( 'cpt_feature_requests' ) );
        ?>
		<a href="<?php 
        echo  esc_attr( $href ) ;

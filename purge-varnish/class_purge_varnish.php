...
/*
  ...
  Version: 2.1
  Author: DP Singh <dev.firoza@gmail.com>
...
class Purge_Varnish {
    ...
    function purge_varnish_validate($post) {
        ...
        $varnish_socket_timeout = (int) $post['varnish_socket_timeout'];
        $varnish_bantype = $post['varnish_bantype'];

        if (empty($varnish_control_terminal) === true) {
            $message['error']['terminal_empty'] = esc_html_e('Varnish Control Terminal can\'t be empty.');
        }

        if (empty($varnish_control_key) === true) {
            $message['error']['key_empty'] = 'Varnish Control Key can\'t be empty.';
        }

        if (empty(trim($post['varnish_socket_timeout'])) === true) {
            $message['error']['terminal_timeout'] = esc_html_e('Varnish connection timeout can\'t be empty.');
        }
        ...
        if (!empty($varnish_control_terminal)) {
            ...
            foreach ($terminals as $terminal) {
                ...
                if (isset($server) && (filter_var($server, FILTER_VALIDATE_IP) === false)) {
                    $message['error']['terminal_' . $server] = $server . ' ' . esc_html_e('is not a valid IP address');
                }
                if (empty($port) || (int) $port <= 0) {
                    $message['error']['terminal_' . $port] = esc_html_e('You need to enter the valid Port Number with IP address');
                }
            }
        }

        if (!is_numeric($varnish_socket_timeout) || $varnish_socket_timeout <= 0) {
            $message['error']['varnish_socket_timeout'] = esc_html_e('Varnish connection timeout must be a positive number.');
        }
        ...
        if (isset($message['error']) && count($message['error'])) {
            ...
            foreach ($message['error'] as $value) {
                $msg .= '<li style="color:#8B0000;">' . $value . '</li>';
            }
            $msg .= '</ul>';
        }
    ...
    function purge_varnish_all_cache_manually() {
        ...
        foreach ($terminals as $terminal) {
            ...
            if ($stats_code == 200) {
                $stats_msg = 'The page url <a href="' . esc_url($_POST['url']) . '" target="@target" style="color:#228B22;"><i>"' . esc_url($parse_url['path']) . '"</i></a> has been purged.';
        ...
        foreach ($terminals as $terminal) {
            ...
            if ($stats_code == 200) {
                if (esc_url($parse_url['path']) == '/') {
                    $stats_msg = 'The <a href="' . esc_url($_POST['url']) . '" target="@target" style="color:#228B22;"><i>"front/home page url"</i></a> has been purged.';
                ...
                $stats_msg = 'The url <a href="' . esc_url($_POST['url']) . '" target="@target" style="color:#8B0000;"><i>"' . esc_url($parse_url['path']) . '"</i></a> has not been purged.';
    ...
    function purge_varnish_comment_post_trigger($ID) {
        ...
        // Call to purge
        $this->purge_varnish_trigger_comment_expire($post);
    }
    ...
    if (is_array($actions)) {
        foreach ($actions as $action) {
            switch ($action) {
                ...
                case 'comment_post':
                    add_action($action, array($purge_varnish, 'purge_varnish_comment_post_trigger'));
                    break;

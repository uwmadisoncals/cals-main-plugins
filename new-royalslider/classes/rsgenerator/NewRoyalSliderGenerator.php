	...
	static function generateSlides($fetch_data, $refresh_cache, $id, $type, $markup = null, $slides = null, $options = null, $template = null, $skin = null, $disable_cache = false) {
		...
		if($arr === false) {
		    ...
		    if($options) {
			    ...
			    $options = apply_filters('new_rs_options', $options, $id);
			    ...
			    if(isset($options['sopts'])) {
			    	...
			    	if ( isset($o['video']['forceMaxVideoCoverResolution']) ) {
			    		unset($o['video']['forceMaxVideoCoverResolution']);
			    	}
		    ...
		    if(is_array($slides) && count($slides) > 0) {
		    	...
		    	$out .= '<div id="'.$css_id.'" class="royalSlider '.$css_id.$skin.$template.'" style="'. $width . $height .';" data-rs-options=\''.esc_attr(json_encode($o)).'\'>' . "\n";

		    	$out = apply_filters( 'new_rs_slides_output_after_start', $out, $id, $type);
			    ...
			    foreach($slides as $key => $slide) {
			    	...
			    	$m->getHelpers()->clear();
				   	...
				   	$slide_html = $m->render($markup, $renderer) ."\n";

				   	$slide_html = apply_filters( 'new_rs_slide_html', $slide_html, $key, $slide, $gen_opts, $options );
			    	$out .= $slide_html;
			    }
	...
	static function preParseOpts($options) {
		...
		if($options) {
            if( isset($options['sopts']) ) {
                ...
                if(!isset($sopts['controlNavigation']) || $sopts['controlNavigation'] == 'none' || $sopts['controlNavigation'] == 'bullets') {
                	$o['thumb_type'] = null;
                } else {
        ...
        $options = array_merge($o_default, $o);
        return $options;
	}

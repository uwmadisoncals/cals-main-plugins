 ...
 */
if ( !class_exists( 'RS_WeDevs_Settings_API' ) ):
    class RS_WeDevs_Settings_API {
    ...
    function callback_hidden( $args ) {
        ...
        $value = esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) );
        $size = isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular';
        $html = sprintf( '<input type="password" disabled class="%1$s-text disabled" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
        echo $html;
    }
    ...
    function callback_instagramauth() {
        echo $args['desc'];
    }
    ...
    function get_option( $option, $section, $default = '' ) {
        ...
        if($option == 'embed' && !isset($options[$option]) ) {
            return array(
                'embed' => array(

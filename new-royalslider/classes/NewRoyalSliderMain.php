...
class NewRoyalSliderMain {
	...
	function __construct( $file ) {
		...
		add_action('rest_api_init', array($this, 'init_private_rest_api'));
	...
	function init() {
		...
		if( function_exists('has_blocks') ) {
			require_once(NEW_ROYALSLIDER_PLUGIN_PATH .  'gutenberg/gutenberg.php');
			$gutenberg = new NewRoyalSliderGutenberg();
		}
	...
	function init_private_rest_api() {
		if ( ! current_user_can( 'edit_posts' ) ) {
			return;
		}

		require_once(NEW_ROYALSLIDER_PLUGIN_PATH .  'gutenberg/gutenberg-rest.php');
		$guten_rest_api = new NewRoyalSlider_Gutenberg_Rest();
	}
	...
	function find_and_register_scripts() {
		...
		if ( !empty($post) &&
			...
			$is_gutenberg_post = true;
		    $blocks = parse_blocks( $post->post_content );

		    // find slider blocks and include their files
			foreach ($blocks as $block) {
				if ( isset($block['blockName']) &&
					 $block['blockName'] === 'new-royalslider/slider' &&
				     !empty($block['attrs']['slider_id']) ) {
					$this->push_script( $block['attrs']['slider_id'] );
				}
			}
		} else {
			$is_gutenberg_post = false;
		}

		if( !$is_gutenberg_post &&
			!$this->load_all &&
			$this->check_post_shortcode ) {
